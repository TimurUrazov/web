...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        String[] requiredMethods = {"before", route.getAction(), "after"};
        HashMap<String, Method> foundMethods = new HashMap<>();
        boolean foundAllMethods = false;
        for (Class<?> clazz = pageClass; clazz != null && !foundAllMethods; clazz = clazz.getSuperclass()) {
            currentMethods = clazz.getDeclaredMethods();
            for (Method method : currentMethods) {
                for (String required : requiredMethods) {
                    if (required.equals(method.getName())) {
                        foundMethods.put(required, method);
                    }
                }
            }
            foundAllMethods = true;
            for (String required : requiredMethods) {
                if (!foundMethods.containsKey(required)) {
        ...
        Method method = foundMethods.get(route.getAction());
        Class<?>[] methodParameterTypes = method.getParameterTypes();
        Object[] invokeParameters = new Object[methodParameterTypes.length];
        for (int i = 0; i < methodParameterTypes.length; i++) {
            if (methodParameterTypes[i] == Map.class) {
                invokeParameters[i] = view;
            } else if (methodParameterTypes[i] == HttpServletRequest.class) {
                invokeParameters[i] = request;
            }
        }

        try {
            for (Method foundMethod : foundMethods.values()) {
                foundMethod.setAccessible(true);
            }
            foundMethods.get("before").invoke(page, request, view);
            method.invoke(page, invokeParameters);
            foundMethods.get("after").invoke(page, request, view);

...
public abstract class BaseRepositoryImpl<T> {
    ...
    private void setInStatement(PreparedStatement statement, Object[] setters) throws SQLException {
        for (int i = 1; i <= setters.length; i++) {
            ...
            if (setters[i - 1] instanceof Boolean) {
                statement.setBoolean(i, (Boolean) setters[i - 1]);
            }
        }
    }
    ...
    public void updateField(String name, Object... setters) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE " + getObject() + " SET " + name + "=? WHERE id=?")) {
                setInStatement(statement, setters);
                if (statement.executeUpdate() != 1) {
                    throw new RepositoryException("Can't update " + name + " field of " + getObject() + ".");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't update " + name + " field of " + getObject() + ".");
